...
'use strict';
var M=[
  {id:'la', nm:'LinAtt',      em:'',   yr:2020,c:'#64748b',t:'L',x:150,y:222,tip:'Replaces softmax with a kernel function. Causal form is a pure RNN: S_t = S_{t-1} + v_t k_t^T with no decay.',dec:'No decay'},
  {id:'rn', nm:'RetNet',      em:'',   yr:2023,c:'#0369a1',t:'L',x:520,y:225,tip:'Scalar \u03b3 < 1 prevents memory explosion. The RetNet decay mask is exactly the ALiBi bias inside softmax.',dec:'Scalar \u03b1'},
  {id:'gla',nm:'GLA',         em:'',   yr:2023,c:'#7c3aed',t:'L',x:572,y:183,tip:'Input-dependent diagonal decay \u03b1_t. First linear model to train as fast as softmax transformers via chunk-wise parallelism.',dec:'Diagonal'},
  {id:'mb', nm:'Mamba',       em:'\uD83D\uDC0D',yr:2023,c:'#0f766e',t:'L',x:560,y:263,tip:'First SSM to match transformers on language modeling. Selective diagonal decay via zero-order hold discretization.',dec:'Diagonal'},
  {id:'dn', nm:'DeltaNet',    em:'',   yr:2024,c:'#059669',t:'L',x:630,y:183,tip:'Delta rule: precisely overwrites stale entries. Non-diagonal transition matrix \u2192 can solve state-tracking tasks.',dec:'Delta rule'},
  {id:'m2', nm:'Mamba-2',     em:'\uD83D\uDC0D',yr:2024,c:'#0891b2',t:'L',x:620,y:225,tip:'State Space Duality (SSD): scalar decay + matrix-multiply training. Bridges Mamba and linear attention.',dec:'Scalar (SSD)'},
  {id:'gdn',nm:'GDN',         em:'',   yr:2024,c:'#d97706',t:'L',x:688,y:263,tip:'Scalar forget gate \u03b1_t \xd7 delta rule: bulk forgetting of stale context + surgical per-entry rewrites.',dec:'\u03b1 \xb7 Delta'},
  {id:'km', nm:'Kimi Linear', em:'',   yr:2025,c:'#0891b2',t:'L',x:790,y:200,tip:'Channel-wise diagonal decay \xd7 delta rule. Used in the KimiLinear hybrid model trained at scale.',dec:'Diag \xb7 Delta'},
  {id:'m3', nm:'Mamba-3',     em:'\uD83D\uDC0D',yr:2026,c:'#0f766e',t:'L',x:840,y:182,tip:'Trapezoidal discretization subsumes causal convolutions. Complex RoPE decay enables state tracking.',dec:'Scalar + RoPE'},
  {id:'g2', nm:'GDN-2',       em:'',   yr:2026,c:'#0e7490',t:'L',x:858,y:228,tip:'Decouples erase gate b_t from write gate w_t. Unifies GDN and Kimi Linear into one expressive framework.',dec:'Diag \xb7 Delta'},
  {id:'swa',nm:'SWA',         em:'',   yr:2020,c:'#475569',t:'S',x:130,y:78, tip:'Sliding window: O(W) KV cache with exact recurrent form. First to bound attention to local context.',dec:'Window'},
  {id:'ab', nm:'ALiBi',       em:'',   yr:2021,c:'#dc2626',t:'S',x:290,y:38, tip:'Fixed linear distance penalty M_ij = \u2212m|i\u2212j| added to attention logits. Extrapolates naturally to longer sequences.',dec:'Scalar bias'},
  {id:'rp', nm:'RoPE',        em:'',   yr:2021,c:'#b45309',t:'S',x:250,y:78, tip:'Rotates Q and K by absolute position so their dot product encodes relative offset. Now the de facto LLM standard.',dec:'Rotary PE'},
  {id:'abc',nm:'ABC',         em:'',   yr:2021,c:'#4f46e5',t:'S',x:310,y:118,tip:'Replaces SWA\'s FIFO eviction with a learned write gate r_t=softmax(W_r x_t). Allows content-based slot routing in a bounded memory.',dec:'Window (learned)'},
  {id:'np', nm:'NoPE',        em:'\uD83E\uDD16',yr:2023,c:'#475569',t:'S',x:500,y:38, tip:'No positional encoding at all. The causal triangular mask alone provides a sufficient implicit positional signal.',dec:'None'},
  {id:'gsa',nm:'GSA',         em:'',   yr:2024,c:'#be185d',t:'S',x:660,y:78, tip:'Adds a sigmoid forget gate \u03c3(Wx_t)^\u03c4 per slot before absorbing new content. Prevents memory saturation in bounded windows.',dec:'Window + gate'},
  {id:'fx', nm:'FoX',         em:'\uD83E\uDD8A',yr:2025,c:'#d97706',t:'S',x:720,y:38, tip:'Input-dependent scalar decay bias f_t \u2208 (0,1) added to softmax logits. Data-driven, input-dependent forgetting.',dec:'Scalar gate'},
  {id:'path',nm:'PaTH',       em:'\uD83D\uDEE3\uFE0F',yr:2025,c:'#1d4ed8',t:'S',x:740,y:78, tip:'Derives position encoding by unrolling DeltaNet: a product of Householder transforms P_{j\u2192t} replaces the dot product k_j^T q_i, giving data-dependent PE.',dec:'Delta (PE)'},
  {id:'wl', nm:'Wall',        em:'\uD83E\uDDF1',yr:2026,c:'#0891b2',t:'S',x:830,y:118,tip:'GLA diagonal gate lifted into softmax via induced-action principle. Per-channel decay. Extrapolates to 160k tokens.',dec:'Diagonal'},
  {id:'raven',nm:'Raven',     em:'\uD83D\uDC26\u200D\u2B1B',yr:2026,c:'#0f172a',t:'S',x:850,y:38, tip:'Sparse router r_t selects which slots to write; per-slot decay a_t controls forgetting. Extrapolates to 16\xd7 training length.',dec:'Window + sparse'},
];
var E=[
...
function renderMask(m) {
  ...
  return `<div class="az-mask-section">
    <div class="az-mask-eq-panel">
      ...
      ${m.mathO?`<div class="az-mask-eq-block"><div class="az-eq-lbl">Readout</div><div class="az-eq-val">\\(${m.mathO}\\)</div></div>`:''}
    </div>
